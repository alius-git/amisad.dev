...
// kind: "step" — one executed step
{
  ...
  "step": {
    ...
    "touches": [ "seller.orders", "ledger.verify" ],   // box ids, §8
...
to instantiate templated boxes, §7) and uses `touches` to fast-poll and
...
2. **`touches`:** add the per-step box-id array from the table in §8.
...
- **Current step card:** step number, persona chip (highlighted when it
  ...
  (`✔`/`✘` + note) exactly like the old console.
...
- **Mini-timeline:** above the footer, a compact read-only replica of the
  actions window's swimlane chart — the same 11 lanes × 69 slots rendered
...
One sequential poll pump (§7.3) serves them all.

| box id | source (GET) | shows | tier |
|---|---|---|---|
| `coordinator.contributions` | `core/30080/v1/needs/contributions` | open needs contributing to aggregation (count) | slow¹ |
...
| `edge-b.runtime` / `edge-b.egress` / `edge-b.ingress` | same, `edge-b/…` | same | slow² |

¹ `GET /v1/needs/contributions` fans out one ledger consent lookup per open
need on every call — real but bounded load; the slow tier keeps it honest
without hammering.
² edge-b receives **no environment traffic in this script** (every need is
...
`scenario-ordinal` matching §6.1 ids, in presentation order.
...
§7.2 — invisibility as feature); troubleshooting (core IP unresolved; 502s;
journal cleared by server restart — open windows detect this and rebuild,
per §5; the buyer-403 consent preflight; data-window staleness when the lab
is down; both-demos-open caveat from §10). Add a **rehearsal note**: the
